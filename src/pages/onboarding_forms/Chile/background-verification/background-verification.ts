...
import { SendData } from './request-form-control'
...
@IonicPage()
@Component({
  selector: 'page-background-verification',
  templateUrl: 'background-verification.html',
})
export class BackgroundVerificationPage {
  ...
  getData(){
    ...
    this._apiService.ChilleBackGroundVerification(apikey,this.login_token).subscribe((res)=>{
      ...
      if(res.success == '1') {
        ...
        console.log(typeof(this.formData.background.userName.is_enable));
        setTimeout(() => {
  ...
  finalSubmit(data : SendData, formStatus:string){
    ...
    this._apiService.chilleSubmitBackgroundVerification(apikey,this.login_token).subscribe((res)=>{
      console.log(res);
      this.loading.dismiss();
      if(res.success == '1') {
        this.apiMessage(res.message);
