...
import { EmergrncyContact, Layout } from "./response";
...
@IonicPage()
@Component({
  selector: "page-india-emg-contact",
  templateUrl: "india-emg-contact.html"
})
export class IndiaEmgContactPage {
  public formData: any;
  employeeId: any;
  login_token: any;
  deviceId: any;
  ...
  editFamilyDetails(data: EmergrncyContact) {
    data.layout = Layout.FORM;
  }

  showConfirm(data: EmergrncyContact) {
    if (
      this.data.length > 1 ||
  ...
  deleteFamilyDetail(data: EmergrncyContact) {
    this.commonLoader();
    let apikey = {
        ...
        res => {
          ...
          if (res.success == "1") {
            ...
            if (this.data.length == 0) {
              this.fill_status = "0";
              let temp: EmergrncyContact = new EmergrncyContact();
              this.data.push(temp);
            }
          } else {
            this.apiMessage(res.message);
  ...
  changeToCard(data: EmergrncyContact) {
    data.layout = Layout.CARD;
  }

  submit(item: EmergrncyContact) {
  ...
  finalSubmit(data: EmergrncyContact, formStatus: string) {
  ...
  closeForm(item: EmergrncyContact) {
    this.data = this.data.filter(element => {
      return element != item;
    });
  }
  ...
  getData() {
    ...
    this._apiService.IndiaGetContactDetail(apikey, this.login_token).subscribe(
      res => {
        this.loading.dismiss();
        if (res.success == "1") {
          console.log(res);
